...
# 运行颜色精度测试
def run_color_accuracy_test():
    """运行颜色精度测试"""
    print("=" * 80)
    print("拼豆颜色识别精度测试")
    print("=" * 80)
    print(f"测试样本数量: {len(test_colors)}")
    print("=" * 80)
    ...
    # 打印测试结果表头
    print(f"{'测试样本':<10} {'原始RGB':<20} {'优化前匹配':<15} {'优化前距离':<15} {'优化后匹配':<15} {'优化后距离':<15} {'结果':<10}")
    print("-" * 80)
    
    for test_color in test_colors:
        # 优化前匹配
        old_result = old_find_nearest_bead_color(test_color["rgb"])
        
        # 优化后匹配
        new_result = new_find_nearest_bead_color(test_color["rgb"])
        
        # 判断是否匹配正确（距离阈值：RGB<50，CIEDE2000<10）
        old_match = old_result["distance"] < 50
        new_match = new_result["distance"] < 10
        ...
        print(f"{test_color['name']:<10} {str(test_color['rgb']):<20} {old_result['color']['name']:<15} {old_result['distance']:<15.2f} {new_result['color']['name']:<15} {new_result['distance']:<15.2f} {old_status}/{new_status:<10}")
    
    print("-" * 80)
    
    # 计算准确率
    old_accuracy = (old_correct / len(test_colors)) * 100
    new_accuracy = (new_correct / len(test_colors)) * 100
    improvement = new_accuracy - old_accuracy
    
    print(f"优化前准确率: {old_accuracy:.1f}% ({old_correct}/{len(test_colors)})")
    print(f"优化后准确率: {new_accuracy:.1f}% ({new_correct}/{len(test_colors)})")
...
if __name__ == "__main__":
    # 运行测试
    result = run_color_accuracy_test()
    
    # 保存测试结果
    with open("accuracy-test-result.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    
    print(f"测试结果已保存到: accuracy-test-result.json")
